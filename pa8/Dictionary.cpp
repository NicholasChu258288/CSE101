...
* Includes Red Black tree functionality.
*********************************************************************************/
...
Dictionary::Node::Node(keyType k, valType v){
    ...
    left = nullptr;
    right = nullptr;
    color = 0; // For context: 0 represents black, 1 represents red
}
...
void Dictionary::LeftRotate(Node* N){
    // R is N right node which will be rotated to the left
    Node* R = N->right;
    ...
    // Links parents
    R->parent = N->parent;
    if (N->parent == nil || N->parent == nullptr){
        root = R;
...
void Dictionary::RightRotate(Node* N){
    // L is N left node which will be rotated to the right
    Node* L = N->left;

    // Rotating L subtree
    N->left = L->right;
    if (L->right != nil && L->right != nullptr){
        L->right->parent = N; 
    }
...
void Dictionary::RB_InsertFixUp(Node* N){
    while (N->parent->color == 1){
        if (N->parent == N->parent->parent->left){
            ...
            if (y->color == 1){
                N->parent->color = 0; // Set parent color to black
                y->color = 0;
...
void Dictionary::RB_DeleteFixUp(Node *N){
    while (N != root && N->color == 0){
        if (N == N->parent->left){
            ...
            if (n->right->color == 0 && n->left->color == 0){
                n->color = 1;
                N = N->parent;
            } else {
                if (n->left->color == 0){
...
void Dictionary::RB_Delete(Node *N){
    Node* y = N;
    int yOgC = y->color;
    Node* x = nil; // Node that will be used for fix up 
    if (N->left == nil || N->left == nullptr){
        x = N->right;
...
void Dictionary::setValue(keyType k, valType v){   
    ...
    N->right = nil;
    N->parent = y;
    N->color = 1; // Color to red, should be set to red by default, just making sure
    if (y == nil || y == nullptr){
        root = N;
    } else if (k < y->key){
        y->left = N;
...
void Dictionary::remove(keyType k){
    // Precondition
    if (contains(k) == false){
        string s = "Dictionary: remove(): key \"" + k + "\"does not exist";
        throw std::logic_error(s);
    }
